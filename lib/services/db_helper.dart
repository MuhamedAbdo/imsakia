...
import 'package:flutter/foundation.dart'; // ضرورية لاستخدام debugPrint
...
class DbHelper {
  ...
  Future<sqlite.Database> initDb() async {
    ...
    if (exists) {
      ...
      try {
        ...
        debugPrint("Error validating database: $e"); // استبدال print
      } finally {
        await checkDb?.close();
      }
      if (!isValid) {
    ...
    if (!exists) {
      try {
        ...
        debugPrint("Error copying database: $e"); // استبدال print
      }
    }
  ...
  Future<void> _createIndexes(sqlite.Database db) async {
    try {
      ...
      debugPrint("Warning: Indexes: $e"); // استبدال print
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getJuzs() async {
    ...
    for (var result in results) {
      ...
      juzs.add({
        'id': result['id'],
        'start_surah_name': startSurah.isNotEmpty
            ? startSurah.first['name_ar']
            : '',
  ...
  Future<List<Map<String, dynamic>>> searchAyahs(String query) async {
    ...
    String normalizedQuery = query
        ...
        .replaceAll('ة', 'ه')
        .replaceAll('ى', 'ي');
    String queryWithoutAL = normalizedQuery.startsWith('ال')
        ? normalizedQuery.substring(2)
        : normalizedQuery;

    const String cleanTextSql =
        "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(a.text, 'ً', ''), 'ٌ', ''), 'ٍ', ''), 'َ', ''), 'ُ', ''), 'ِ', ''), 'ّ', ''), 'ْ', ''), 'ٰ', ''), 'أ', 'ا'), 'إ', 'ا'), 'آ', 'ا'), 'ة', 'ه')";
    const String cleanNameSql =
        "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(s.name_ar, 'ً', ''), 'ٌ', ''), 'ٍ', ''), 'َ', ''), 'ُ', ''), 'ِ', ''), 'ّ', ''), 'ْ', ''), 'ٰ', ''), 'أ', 'ا'), 'إ', 'ا'), 'آ', 'ا'), 'ة', 'ه')";
