import 'dart:async';
import 'dart:developer' as developer; // إضافة هذا الاستيراد للمسجل (Logger)
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              backgroundColor: Theme.of(
                context,
              ).colorScheme.primary.withValues(alpha: 0.9), // تم التعديل هنا
            ),
          );
        }
      });
      ...
      child: Scaffold(
        ...
        body: PageView.builder(
          ...
          itemBuilder: (context, index) {
            final qProvider = Provider.of<QuranProvider>(
              context,
              listen: false,
            );
            return JuzPageItem(
...
class _JuzPageItemState extends State<JuzPageItem> {
    ...
    if (widget.initialSurahId != null) {
      ...
      try {
        targetAyahData = _ayahItems.firstWhere(
          (a) =>
              a['surah_id'] == widget.initialSurahId &&
              a['number_in_surah'] == widget.initialAyah,
        );
      } catch (_) {}
    } else {
      try {
        targetAyahData = _ayahItems.firstWhere(
          (a) => a['number_in_surah'] == widget.initialAyah,
        );
        targetKey =
            '${targetAyahData['surah_id']}_${targetAyahData['number_in_surah']}';
      } catch (_) {}
    }
    ...
    setState(() {
      ...
      if (!_isAutoScrolling) {
        ...
        try {
          ...
          developer.log("Wakelock Error: $e"); // تم استبدال print هنا
        }
      } else {
        try {
          WakelockPlus.enable();
    ...
    for (var ayah in _ayahItems) {
                ...
                child: VisibilityDetector(
                  ...
                  onVisibilityChanged: (info) {
                    if (info.visibleFraction > 0.1 &&
                        mounted &&
                        _currentVisibleSurah != surahNameAr) {
      ...
      spans.add(
        TextSpan(
          children: [
            WidgetSpan(
              child: SizedBox(width: 0, height: 0, key: _ayahKeys[keyStr]),
            ),
            TextSpan(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          children: [
            ...
            Text(
              "${ayah['surah_name_ar'] ?? ''} - آية ${ayah['number_in_surah']}",
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: Theme.of(context).colorScheme.primary
                              .withValues(alpha: 0.3), // تم التعديل
                        ),
                      ),
                      child: Text(
                        _cleanAyahText(
    ...
    return Scaffold(
      appBar: AppBar(
              ...
              Text(
                ...
                style: GoogleFonts.tajawal(
                  color: isDarkMode
                      ...
                      : Colors.white.withValues(alpha: 0.8), // تم التعديل
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(
              qProvider.lastJuzId == widget.juzId && qProvider.isJuzMode
                  ? Icons.bookmark
          ...
          if (_isAutoScrolling && _showSpeedSlider)
            Container(
              ...
              decoration: BoxDecoration(
                color: Theme.of(
                  context,
                ).cardColor.withValues(alpha: 0.95), // تم التعديل
  ...
  void dispose() {
    try {
      ...
      developer.log("Wakelock Dispose Error: $e");
    }
    _scrollTimer?.cancel();
